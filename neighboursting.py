...
class NeighborsModel:
    ...
    def generate_new_world(self):
        ...
        for row_num in range(self.SIDE):
            ...
            for col_num in range(self.SIDE):
                update_actor_place = self.actor_action(row_num, col_num, none_list_coordinates)
    ...
    def neighbourhood_check(self, row_num, col_num, none_list_coordinates):
        ...
        if self.world[row_num][col_num] == Actor.RED:
            ...
            satisfiedröd = self.satisfiedred(nr_of_none,nr_of_blue)
            if satisfiedröd < 0.8:
                unsatisfied.append([row_num,col_num])

        if self.world[row_num][col_num] == Actor.BLUE:
            satisfiedblå = self.satisfiedblue(nr_of_none,nr_of_red)
            if satisfiedblå < 0.8:
                unsatisfied.append([row_num,col_num])

    def satisfiedred(self,nr_of_none,nr_of_blue):
    ...
    def satisfiedblue(self,nr_of_none,nr_of_red):
        colortotal = 8 - nr_of_none
        if colortotal == 0:
            colortotal = 1
    ...
    def switch_pos(self,none_list_coordinates, unsatisfied):
        while len(none_list_coordinates) > 0:
            templist[none_list_coordinates[i][0]][none_list_coordinates[i][1]] = self.world[none_list_coordinates[i][0][none_list_coordinates[i][1]]
            
       # self.update_actor(self, nr_of_blue, nr_of_red, nr_of_none)

    #def update_actor(self,nr_of_blue, nr_of_red, nr_of_none):
     #   for i in range(start_row_num,end_row_num):
      #      for j in range(start_col_num, end_col_num):
       #     if self.world[i][j] == Actor.RED:
        #        posx = random()
         #       posy = random()
          #      while self.world[posx][posy] != 0 and newmatrix[posx][posy]!=0:
           #         posx = random()
            #        posy = random()
             #   newmatrix[posx][posy] = BLUE
              #  satisfied = 1 - (nr_of_blue/(nr_of_blue + nr_of_red))
